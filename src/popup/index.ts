...
import {
  ...
  resetSimulcastDynamicSyncState,
  type SimulcastDynamicSyncSample,
} from './simulcast-dynamic-sync';
import type {
  StrictDelayedVideoRelayMessage,
  StrictVideoCrop,
} from '@/offscreen/simulcast-video-relay';
...
function getErrorMessage(error: unknown): string {
  ...
  if (
    ...
    return '当前页面不能被捕获。请切到普通网页/视频页，并从该页面打开扩展后再开始同传。';
  }
  return message;
}
...
function getSimulcastVideoSyncModeControlValue(): SimulcastVideoSyncMode {
  return normalizeSimulcastVideoSyncMode(
    getControlValue('simulcast-video-sync-mode', 'strict-delayed-player')
  );
}

// 精准同步抓不到标签页视频轨时，本次会话回退页面视频对齐；启动/停止重置。
let simulcastStrictFellBack = false;

/** 运行时实际生效的视频同步模式：精准同步回退后按页面视频对齐。 */
function effectiveSimulcastVideoSyncMode(): SimulcastVideoSyncMode {
  const requested = getSimulcastVideoSyncModeControlValue();
  if (requested === 'strict-delayed-player' && simulcastStrictFellBack) {
    return 'fallback-page-video';
  }
  return requested;
}

function getStrictBufferControlValue(): number {
  // 注意：不能用 getNumberControlValue（它会把值钳到 0~1，那是给音量用的）。
  return normalizeStrictBufferSec(
    Number(getControlValue('simulcast-strict-buffer-sec', String(DEFAULT_STRICT_BUFFER_SEC)))
  );
}

function isStrictVideoCrop(value: unknown): value is StrictVideoCrop {
  if (!value || typeof value !== 'object') {
    return false;
  }
  const rect = value as Partial<Record<keyof StrictVideoCrop, unknown>>;
  return (
...
async function getActiveVideoViewportRect(): Promise<StrictVideoCrop | undefined> {
  try {
    const response = await sendMessageToActiveTab<{
      rect?: StrictVideoCrop | null;
    }>({ type: 'simulcast:getVideoViewportRect' } as unknown as ChromeMessageRequest);
    const rect = response.rect ?? response.data?.rect;
    return isStrictVideoCrop(rect) ? rect : undefined;
  } catch {
    return undefined;
  }
}
...
function assertCapturableTab(tab: chrome.tabs.Tab | null): asserts tab is chrome.tabs.Tab {
  ...
  if (
    ...
    throw new Error('Chrome/浏览器内置页面不能被捕获。请切到普通网页或视频页面后再开始同传。');
  }
}
...
function handleSimulcastUpdate(
  ...
  // 精准同步抓不到标签页视频轨 → 本次会话回退页面视频对齐，并按页面流程 hold 等首段译音。
  if (
    message?.strictVideo === 'unavailable' &&
    simulcastRunning &&
    !simulcastStrictFellBack &&
    getSimulcastVideoSyncModeControlValue() === 'strict-delayed-player'
  ) {
    simulcastStrictFellBack = true;
    void holdVideoUntilTranslatedAudio().finally(() => {
      setSimulcastSyncStatus('无法捕获标签页视频画面，已回退为页面视频对齐');
    });
    sendResponse({ status: 'ok' });
    return;
  }
  ...
  if (
    ...
    const sourceStartWallMs = Date.now();
    observeSourceSubtitleAnchor(simulcastDynamicSyncState, sourceStartWallMs);
    if (simulcastFirstSourceWall === 0) {
      simulcastFirstSourceWall = sourceStartWallMs;
    }
  }
  ...
  if (message?.translatedAudio?.event === 'playback-started') {
    handleTranslatedAudioPlaybackStarted(message.translatedAudio);
  }
  ...
  if (
    ...
    if (!pushLiveSubtitleToStrictPlayer('source', '', event)) {
      overlaySource = '';
      overlayTranslation = '';
      sendSubtitleToVideo(true);
    }
  }
  ...
  if (
    ...
    if (!pushLiveSubtitleToStrictPlayer('translation', '', event)) {
      overlayTranslation = '';
      sendSubtitleToVideo(!overlaySource);
    }
  }
  ...
  if (SIMULCAST_SOURCE_SUBTITLE_EVENTS.has(event)) {
    ...
    if (simulcastRunning && !transcribeActive) {
      if (pushLiveSubtitleToStrictPlayer('source', text, event)) {
        // strict player 由播放器侧按当前缓冲延迟显示，避免原页面播放头与延迟画面错位。
      } else if (simulcastLatestVideoClock) {
  ...
  if (SIMULCAST_TRANSLATION_SUBTITLE_EVENTS.has(event)) {
    ...
    if (simulcastRunning && !transcribeActive) {
      if (pushLiveSubtitleToStrictPlayer('translation', text, event)) {
        // strict player 由播放器侧按当前缓冲延迟显示，避免原页面播放头与延迟画面错位。
      } else if (simulcastLatestVideoClock) {
        syncSubtitleToPlayhead();
      } else {
...
async function handleSimulcastStartClick(): Promise<void> {
  try {
    ...
    const activeTab = await getActiveTab();
    assertCapturableTab(activeTab);
    const videoViewportRectPromise =
      videoSyncMode === 'strict-delayed-player'
        ? getActiveVideoViewportRect()
        : Promise.resolve(undefined);
    setTranslationStatus('simulcast-status', '正在创建当前标签页音频捕获会话...', 'info');

    const videoViewportRect = await videoViewportRectPromise;
    ...
    >({
      ...
      originalVolume: getNumberControlValue('simulcast-original-volume', 0),
      ...
      videoSyncMode,
      videoViewportRect,
      strictPlayerBufferSec: getStrictBufferControlValue(),
    ...
    const strictVideoUnavailable =
      videoSyncMode === 'strict-delayed-player' && simulcast?.strictVideo === 'unavailable';
    setStrictPlayerSubtitleSession(
      videoSyncMode === 'strict-delayed-player' && !strictVideoUnavailable
        ? simulcast?.strictPlayerSessionId
        : undefined
    );

    setTranslationStatus(
      'simulcast-status',
      strictVideoUnavailable
        ? '已开始同声传译；无法捕获标签页视频画面，已回退为页面视频对齐。'
    ...
    simulcastStrictFellBack = false;
    simulcastFirstSourceWall = 0;
    simulcastSyncMeasured = false;
    simulcastSyncMeasuredFrom = 'none';
    resetSimulcastDynamicSyncState(simulcastDynamicSyncState);
    simulcastLatestVideoClock = null;
    updateSimulcastControls();
    if (strictVideoUnavailable) {
      simulcastStrictFellBack = true;
      void holdVideoUntilTranslatedAudio().finally(() => {
        setSimulcastSyncStatus('无法捕获标签页视频画面，已回退为页面视频对齐');
      });
...
function updateSimulcastControls(): void {
  ...
  start?.classList.toggle('col-span-2', false);
  stop?.classList.remove('hidden');
  stop?.classList.toggle('col-span-2', simulcastRunning);
}

async function refreshSimulcastRuntimeStatus(): Promise<void> {
  try {
    const response = await safeSendRuntimeMessage<
      unknown,
      {
    ...
    >({ type: 'simulcast:getStatus' });
    const simulcast = response.simulcast ?? response.data?.simulcast;
    simulcastRunning = simulcast?.state === 'capturing';
    simulcastStrictFellBack = simulcastRunning && simulcast?.strictVideo === 'unavailable';
    if (simulcastRunning && simulcast?.strictPlayerSessionId && !simulcastStrictFellBack) {
      setStrictPlayerSubtitleSession(simulcast.strictPlayerSessionId);
    }
    updateSimulcastControls();
  } catch {
    updateSimulcastControls();
...
function resetSimulcastLocalState(options: {
  ...
  clearStrictPlayerSubtitleSession();
...
async function updateSimulcastPlaybackSettings(): Promise<void> {
  ...
  if (response.status === 'error') {
    throw new Error(response.error || '更新同传音量失败');
  }
}

function bindSimulcastPlaybackControl(id: string): void {
  ...
  const handleChange = (): void => {
    void updateSimulcastPlaybackSettings().catch((error) => {
      setTranslationStatus('simulcast-status', getErrorMessage(error), 'error');
    });
  };
...
const simulcastDynamicSyncState = createSimulcastDynamicSyncState();
let simulcastLatestVideoClock: NonNullable<SimulcastUpdateMessage['videoClock']> | null = null;
const SIMULCAST_TRANSLATED_AUDIO_PLAYING_STATUS = '同声传译运行中：正在播放译音。';

// 视频上字幕叠加：当前句的原文/译文
let overlaySource = '';
let overlayTranslation = '';
let strictPlayerSubtitleChannel: BroadcastChannel | null = null;
let simulcastStrictPlayerSessionId = '';
...
function sendSubtitleToVideo(clear = false): void {
  ...
  if (clear || !strictPlayerSubtitleChannel) {
  ...
  sendSubtitleToStrictPlayer(clear, mode);
}

function setStrictPlayerSubtitleSession(sessionId: string | undefined): void {
  if (!sessionId) {
    clearStrictPlayerSubtitleSession();
    return;
  }
  if (simulcastStrictPlayerSessionId === sessionId && strictPlayerSubtitleChannel) {
    return;
  }
  clearStrictPlayerSubtitleSession();
  simulcastStrictPlayerSessionId = sessionId;
  strictPlayerSubtitleChannel = new BroadcastChannel(getSimulcastStrictPlayerChannel(sessionId));
  void sendMessageToActiveTab({
    type: 'simulcast:subtitle',
    clear: true,
  }).catch(() => undefined);
  sendSubtitleToStrictPlayer(
    !overlaySource && !overlayTranslation,
    getControlValue('simulcast-subtitle-mode', 'bilingual')
  );
}

function clearStrictPlayerSubtitleSession(): void {
  strictPlayerSubtitleChannel?.close();
  strictPlayerSubtitleChannel = null;
  simulcastStrictPlayerSessionId = '';
}

function sendSubtitleToStrictPlayer(clear: boolean, mode: string): void {
  if (!strictPlayerSubtitleChannel) {
    return;
  }
  strictPlayerSubtitleChannel.postMessage({
    type: 'subtitle',
    clear,
    source: overlaySource,
    translation: overlayTranslation,
...
function pushLiveSubtitleToStrictPlayer(
  channel: SimulcastSubtitleChannel,
  text: string,
  event: number
): boolean {
  ...
  strictPlayerSubtitleChannel.postMessage({
    ...
    text,
    event,
    reset: SIMULCAST_SUBTITLE_START_EVENTS.has(event),
    source: overlaySource,
    translation: overlayTranslation,
    mode: getControlValue('simulcast-subtitle-mode', 'bilingual'),
  } satisfies StrictDelayedVideoRelayMessage);
  return true;
}
...
async function sendVideoSync(
  ...
  const videoSyncMode = effectiveSimulcastVideoSyncMode();
  if (enabled && videoSyncMode === 'strict-delayed-player') {
    // 精准同步缓冲固定（启动设定 + 滑块手调），不被自动测量/动态延迟改写——这是音画锁步的前提。
    if (!options.silent) {
      setSimulcastSyncStatus('精准同步：固定缓冲已生效');
    }
    return;
  }
  ...
  try {
    ...
    if (videoFound === false) {
      setSimulcastSyncStatus(
        videoCount === 0 ? '页面没有视频元素（无法对齐）' : '未找到主视频（在有视频的页面再试）'
      );
...
function describeDynamicSyncAction(action: unknown): string {
  switch (action) {
    case 'rate':
      return '微调播放速度';
    case 'seek':
...
async function sendDynamicVideoSync(sample: SimulcastDynamicSyncSample): Promise<void> {
  // 精准同步用固定缓冲 + offscreen 主时钟锁步，不接受动态测量改写缓冲（否则会破坏锁步）。
  if (effectiveSimulcastVideoSyncMode() === 'strict-delayed-player') {
    return;
  }
  ...
  try {
    const resp = await sendMessageToActiveTab<{
      ...
      type: 'simulcast:dynamicVideoSync',
      targetDelaySec: sample.targetDelaySec,
      releaseHold: simulcastVideoHoldPending,
    });
    const videoFound = resp?.videoFound ?? resp?.data?.videoFound;
    const action = resp?.action ?? resp?.data?.action;
    if (videoFound === false) {
      setSimulcastSyncStatus('未找到主视频（无法动态对齐）');
      return;
    }
    const videoClockText =
      simulcastLatestVideoClock && typeof simulcastLatestVideoClock.currentTime === 'number'
        ? `，视频 ${simulcastLatestVideoClock.currentTime.toFixed(1)}s`
        : '';
    setSimulcastSyncStatus(
      `自动动态对齐 ${sample.targetDelaySec.toFixed(1)}s，实测 ${sample.rawDelaySec.toFixed(1)}s（${describeDynamicSyncAction(
        action
      )}，样本 ${sample.sampleCount}${videoClockText}）`
    );
  } catch (error) {
    setSimulcastSyncStatus('动态同步失败：' + getErrorMessage(error));
  } finally {
    if (simulcastVideoHoldPending) {
      simulcastVideoHoldPending = false;
    }
...
function handleTranslatedAudioPlaybackStarted(
  translatedAudio: NonNullable<SimulcastUpdateMessage['translatedAudio']>
): void {
  if (!simulcastRunning) return;
  const wallTimeMs =
    typeof translatedAudio.wallTimeMs === 'number' && Number.isFinite(translatedAudio.wallTimeMs)
      ? translatedAudio.wallTimeMs
      : Date.now();
  const sample = observeTranslatedAudioPlayback(simulcastDynamicSyncState, wallTimeMs);
  if (!sample) {
    maybeAutoMeasureSyncDelay('audio', wallTimeMs);
    return;
  }

  simulcastSyncMeasured = true;
  simulcastSyncMeasuredFrom = 'audio';
  void sendDynamicVideoSync(sample);
}

/**
 * 自动对齐 fallback：首译文到达时先预估；译音真正开始播放后走动态样本闭环。
 * 不用 subtitle.startTime（火山实时常返回 0/会话相对值，会让延迟无限增大→疯狂 seek/循环）。
 */
function maybeAutoMeasureSyncDelay(
  source: Exclude<SimulcastSyncMeasurementSource, 'none'>,
  ...
  const lagSec = (observedWallMs - simulcastFirstSourceWall) / 1000;
...
function initializeTranslationActions(): void {
  ...
  // 精准同步缓冲滑块：运行中改值且为精准模式 → 即时下发新缓冲（视频中继 + 译音主时钟同步更新）。
  document.getElementById('simulcast-strict-buffer-sec')?.addEventListener('change', () => {
    if (simulcastRunning && effectiveSimulcastVideoSyncMode() === 'strict-delayed-player') {
      void sendStrictPlayerDelay(getStrictBufferControlValue()).catch(() => undefined);
    }
  });
  updateSimulcastControls();
  void refreshSimulcastRuntimeStatus();
...
function setupMessageListeners() {
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    const notificationHandlers: Record<string, (m: any, r: (resp: any) => void) => void> = {
      ...
      'simulcast:videoClock': handleSimulcastUpdate,
    };
...
function initializeSimulcastSettings(): void {
  void (async () => {
    ...
    setControlValue(
      'simulcast-original-volume',
      normalizeInitialSimulcastOriginalVolume(simulcast.originalVolume)
    );
...
const LEGACY_SIMULCAST_ORIGINAL_VOLUME_DEFAULT = 0.25;

function normalizeInitialSimulcastOriginalVolume(value: number): number {
  // 旧版本默认回放 25% 原声，会和译音互相干扰；把旧默认值迁到静音，保留其他手动值。
  return Math.abs(value - LEGACY_SIMULCAST_ORIGINAL_VOLUME_DEFAULT) < 0.0001 ? 0 : value;
}
